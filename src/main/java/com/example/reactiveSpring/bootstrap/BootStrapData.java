package com.example.reactiveSpring.bootstrap;
...
public class BootStrapData implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        beerRepository.count().subscribe(count -> {
            System.out.println("Count for beer repository is: " + count);
        });

        customerRepository.count().subscribe(count -> {
            System.out.println("Count for customer repository is: " + count);
        });
    }
        ...
        customerRepository.count().subscribe(count -> {
            if (count == 0) {
                Customer customer1 = Customer.builder()
                        .customerName("Akshaan Khanna")
                        .createdDate(LocalDateTime.now())
                        .lastModifiedDate(LocalDateTime.now())
                        .build();

                Customer customer2 = Customer.builder()
                        .customerName("Arvind Raju")
                        .createdDate(LocalDateTime.now())
                        .lastModifiedDate(LocalDateTime.now())
                        .build();

                Customer customer3 = Customer.builder()
                        .customerName("Jisbin Joseph")
                        .createdDate(LocalDateTime.now())
                        .lastModifiedDate(LocalDateTime.now())
                        .build();
